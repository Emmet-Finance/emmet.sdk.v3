...
type LPDeposit_init_args = {
  $$type: "LPDeposit_init_args";
  master: Address;
  owner: Address;
};

function initLPDeposit_init_args(src: LPDeposit_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
...
const LPDeposit_errors: { [key: number]: { message: string } } = {
...
const LPDeposit_getters: ABIGetter[] = [
  {
    name: "amount",
    arguments: [],
...
const LPDeposit_receivers: ABIReceiver[] = [
...
export class LPDeposit implements Contract {
  ...
  async send(
    ...
    if (
      ...
      body = beginCell().store(storeSetPosition(message)).endCell();
    }
    ...
    if (
      ...
      body = beginCell().store(storeWithdrawTokens(message)).endCell();
    }
    if (
      message &&
      typeof message === "object" &&
